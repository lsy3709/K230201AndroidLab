...
class MainActivity : AppCompatActivity() { // AppCompatActivity 부모 클래스 로 부터 상속 받음.
    ...
    override fun onCreate(savedInstanceState: Bundle?) { // onCreate 액티비티의 생명주기에서 더 설명. 13장.
        ...
        // 그래서, 코드에서 리소스의 값을 불러올 때 항상. R.layout 등으로 시작을해서 불러 옵니다.
        // 화면을 출력 하겠다. 레이아웃7장 더 설명
        // (뷰 그룹 : 뷰(텍스트, 이미지, 인풋, 체크박스, 라디오..)를 모아주는 역할
        //setContentView(R.layout.activity_main)

        //뷰 바인딩 기술을 통해서, 뷰들을 , 특정 바인딩이라는 객체에 모두 모아서,
        // 접근을 쉽게 해주는 기술.
        // 사용하기전에 항상, build.gradle 파일에 설정 훅, sync now 적용해서 사용해야함.

        // 이문법은 자동으로 생성된 바인딩 파일을 inflate 함수와 , 인자는 :layoutInflater 고정
